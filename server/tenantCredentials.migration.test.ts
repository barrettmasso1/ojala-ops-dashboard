...
const migration = readFileSync("drizzle/0013_tenant_credential_and_frigate_event_boundaries.sql", "utf8");
...
describe("tenant credential migration packaging", () => {
  it("registers a separate migration after Phase 1 without changing 0012", () => {
    const entry = journal.entries.find((item: { tag: string }) => item.tag === "0013_tenant_credential_and_frigate_event_boundaries");
  ...
  it("is non-destructive and adds active-store, verifier, and source-event controls", () => {
    expect(migration).toContain("ALTER TABLE `stores` ADD `isActive` int DEFAULT 1 NOT NULL");
    expect(migration).toContain("`credentialVerifier` varchar(512) NOT NULL");
    expect(migration).toContain("`revokedAt` timestamp");
    expect(migration).toContain("`sourceEventId` varchar(128)");
    expect(migration).toContain("SET `sourceEventId` = CONCAT('legacy-', `id`), `sourceEventAt` = `receivedAt`");
