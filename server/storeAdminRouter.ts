...
import {
  getStoreHandoffZone, getStoreProfile, listStoreCredentials, listStoreDrafts, provisionStoreDraft,
  rotateStoreCredential, updateStoreProfile,
  saveStoreHandoffZone,
} from "./storeAdminDb";
...
const handoffZoneGeometry = z.object({
...
export const storeAdminRouter = router({
  ...
  handoffZone: adminProcedure.query(({ ctx }) => getStoreHandoffZone(ctx.user.storeId)),
  saveHandoffZone: adminProcedure.input(handoffZoneGeometry).mutation(
    ({ ctx, input }) => saveStoreHandoffZone(ctx.user.storeId, input),
  ),
